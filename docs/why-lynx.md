...
**Lynx sits between your AI agent and the real world.** Every action the agent wants to take (shell command, SQL query, HTTP call, file write) is checked against a YAML policy you wrote. Dangerous actions are blocked, irreversible ones are previewed first, the rest run normally. Every decision is recorded in a hash-chained audit log. Without Lynx, hallucinated commands run as-is. With Lynx, **only policy-approved actions touch the world.**

## When you need Lynx

You need Lynx when **any** of these are true:

| Situation | Why Lynx helps |
|-----------|------------------|
...
| You're shipping an LLM-powered product to customers | "The model decided to" isn't a defense. Lynx gives you a paper trail of what was decided, why, and who approved it |
...
## How Lynx compares to alternatives
...
| **Lynx** | Policy + durability + audit *at the tool-call boundary* | Cluster orchestration; observability of LLM token-level traces |

**Lynx's niche is the tool-call boundary** — the single place where an agent's intention meets the world. Tools above it (frameworks) keep planning; tools below it (Temporal, OPA) keep coordinating. Nobody else owns the seat-belt at that exact layer.

## Real incidents Lynx would have prevented

These all happened in 2025–2026:

1. **AWS engineer's prod environment wiped** by an agent asked to fix a minor Cost Explorer issue. With Lynx's `require_approval` rule on production destructive ops, the rebuild would have prompted for sign-off.
2. **Meta AI safety director's inbox deleted** by her own agent; "stop" signals were ignored. Lynx owns the stop signal at the kernel level — not "whenever the model decides to listen."
...
4. **n8n upgrade broke function-calling schemas silently across the user base.** Tool contracts in Lynx are versioned and pinned to the run.
...
## What Lynx is *not* trying to be

| Not | Because |
|-----|---------|
| A new agent framework | LangGraph / CrewAI / OpenAI SDK already exist; we wrap them, not replace them |
| A prompt-injection defense | Lynx stops unsafe *actions*; it doesn't reason about whether the agent's *intention* was corrupted by a hostile document. Treat LLM outputs as untrusted; let policy be the second line of defense. |
| A general-purpose RBAC system | Policy is for tool calls. Use your existing auth layer for user permissions. |
| A model evaluation framework | Use Braintrust, LangSmith, or Arize Phoenix for that. Lynx is downstream — it shows what *happened*, not whether the LLM was *good*. |

## Summary in one line

> Lynx is the seat belt. Your LLM is the driver. You still pick the destination — but if the driver hallucinates, the belt holds.

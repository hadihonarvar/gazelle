...
Off by default. Enable with::
    ...
    from lynx.observability import enable_prometheus, enable_otel
...
def enable_prometheus(port: int = 9100) -> None:
    ...
    Requires `pip install lynx-agent[prometheus]` (or `pip install prometheus-client`).

    Exposes the following metrics:
        lynx_steps_total{verdict="allow|deny|dry_run|approve_required|transform"}
        lynx_step_duration_seconds{tool=...}
        lynx_runs_total{status="succeeded|failed|paused|cancelled"}
        lynx_actions_dropped_total{reason="denied|approval_timeout"}
    """
    ...
    STEPS_TOTAL = Counter("lynx_steps_total", "Total steps by verdict", ["verdict"])
    STEP_DURATION = Histogram("lynx_step_duration_seconds", "Step duration", ["tool"])
    RUNS_TOTAL = Counter("lynx_runs_total", "Total runs by terminal status", ["status"])
    ACTIONS_DROPPED = Counter("lynx_actions_dropped_total", "Actions not executed", ["reason"])
...
def enable_otel(service_name: str = "lynx") -> None:
    """Initialize OpenTelemetry tracing.

    Requires `pip install lynx-agent[otel]`. Honors the standard OTEL_EXPORTER_*
    ...
    _otel_tracer = trace.get_tracer("lynx")
...
def trace_step(run_id: str, seq: int, tool: str):
    ...
    with _otel_tracer.start_as_current_span(
        "lynx.step",
        attributes={"lynx.run_id": run_id, "lynx.step_seq": seq, "lynx.tool": tool},
    ):
        yield
